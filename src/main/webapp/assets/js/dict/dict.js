            	...
            	operateEvents = {
        		        ...
        		        'click .editDict': function (e, value, row, index) {
        		        	...
        		        	core.openModel('modal-EditDict','修改字典',function(){
        		        		...
        		        		$("#tatil").next("h3").html("编辑字典          "+row.value);
...
//		        		});
    			...
    			$("#EditbtnSubmit").click(function(){
    				...
    				if(value.length<0||type.length<0){return };
    				$.ajax({
						url: F.basepath+'/dict/editDict',
	        			type:'POST',
	        			data:{id:valueId,value:value,parentId:parentId,type:type,remark:remark},
	        			success:function(data){
	        				if(data.result>0){
	        					core.closeModel('modal-EditDict');
	        					F.table.reload();
    			...
    			$('#btnSubmit').click(function(){    			
    				...
    				if(value.length>0&&type.length>0){
    					$.ajax({
    	        			...
    	        			type:'POST',
    	        			data:{value:value,parentId:parentId,type:type,remark:remark},
    	        			success:function(data){
    	        				if(data.result>0){
    	        					core.closeModel('modal-DictTree');
    	        					F.table.reload();
...
//            	if (base.perList.user.edit_dep) {
            		_btnAction += "<a class='startDict btn btn-primary btn-small' href='#' title='启用或停用' style='margin-left:5px'>"+(row.status==1?"停用":"启用")+"</a>";
//            	}
//            
...
//            	}
            	...
            	console.log(id);
            	base.ajaxRequest(F.basepath+'/dict/delete',{"id":id},function(data){
