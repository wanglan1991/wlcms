package com.ekt.cms.utils.taobao.api.security;
...
import com.ekt.cms.utils.taobao.api.internal.util.Base64;
import com.ekt.cms.utils.taobao.api.internal.util.LruHashMap;
import com.ekt.cms.utils.taobao.api.internal.util.NamedThreadFactory;
...
public class SecurityCore {
    ...
    public SecretContext getSecret(String session, Long secretVersion) throws ApiException {
        SecretContext secretContext = getSecret(generateSecretKey(session, secretVersion));
    ...
    private SecretContext getSecret(String session) {
        ...
        if (session != null) {
            secretContext = appUserSecretCache.get(session);
        } else {
            secretContext = SecurityCore.appSecretContext;
        }
        return secretContext;
    }
    ...
    private SecretContext callSecretApi(String session, Long secretVersion) throws ApiException {
        ...
        if (response.isSuccess() && response.getSecret() != null) {
            ...
            if (session != null) {
                appUserSecretCache.put(generateSecretKey(session, secretVersion), secretContext);
            } else {
                SecurityCore.appSecretContext = secretContext;
            }
            return secretContext;
        } else {
